...
def generate_initial_data(ag_data, example): 
    ...
    for idx in range(adj_m.shape[0]):
        # Escalation setup: ALL agents share the SAME (normal) system prompt so node
        # features are identical; the attack payload is delivered via the adv_texts
        # context (turn 0) and the per-turn escalation prompts instead.
...
if __name__ == "__main__":
    ...
    def parse_arguments():
        ...
        parser.add_argument("--dataset", type=str, default="memory_attack_v2")
