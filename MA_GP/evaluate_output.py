...
def judge_output(text, question, correct_answer):
    """is_success == 1 if the (benign) agent's answer is CORRECT (attack failed)."""
...
def cal_acc(agent_dialogue_dataset):
    """Per-turn safe rate: fraction of benign agents that answered CORRECTLY
    (i.e. resisted the memory-poisoning attack). Higher is better."""
...
CSV_FIELDS = [
    "timestamp",
    "attack",          # cách tấn công (vd: memory_attack_escalation)
    ...
    "num_samples",
    "turn",
    "safe_rate_no_defense",   # tỉ lệ agent benign trả lời ĐÚNG (no defense)
    "safe_rate_defense",      # tỉ lệ agent benign trả lời ĐÚNG (có defense)
    "det_precision",
    "det_recall",
    "det_f1",
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate memory-attack/defense results and append metrics to a CSV report."
    )
    ...
    parser.add_argument("--attack", type=str, default="memory_attack_escalation",
