...
# Default agents (csqa/mmlu) imported for type hints; gsm8k swaps these inside main().
...
async def no_defense_communication(ag, qa_data, num_dialogue_turns):
    communication_data = []
...
async def defense_communication(ag, gnn, qa_data, adj_m: np.ndarray, num_dialogue_turns):
    communication_data = []
...
async def main():
    ...
    # Dataset-aware agents: gsm8k uses a different attack/agent variant.
    ...
    for _idx, d in enumerate(tqdm(dataset)):
        ...
        try:
            ...
            communication_data_defense, identified_attackers = await defense_communication(agwd, gnn, qa_data, adj_m, num_dialogue_turns)
