...
def generate_initial_data(ag_data, qa_data): 
    ...
    for idx in range(adj_m.shape[0]):
        # Escalation: all agents share the SAME (normal) system prompt -> identical node
        # features; the attack payload is delivered via qa_data + per-turn escalation prompts.
