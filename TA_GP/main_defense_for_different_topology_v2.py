...
def parse_arguments():
    ...
    parser.add_argument("--model_type", type=str, default="hugging-quants/Meta-Llama-3.1-70B-Instruct-AWQ-INT4")
    ...
    filename_no_defense = f"{current_time_str}-no_defense-model_type_{args.model_type.replace('/', '_')}.json"
    filename_defense = f"{current_time_str}-defense-model_type_{args.model_type.replace('/', '_')}.json"
...
async def main(): 
    ...
    max_turns = len(dataset[0]["communication_data"])  # total rounds including initial
    ...
    for _idx, d in enumerate(tqdm(dataset)):
        ...
        attacker_instruction = d["attacker_instruction"]
        agent_scratchpad = d["agent_scratchpad"]
        case = (user_instruction, available_tool_names, available_tool_descriptions, agent_scratchpad, attacker_instruction)
        ...
        try:
            ...
            communication_data_defense, identified_attackers = await defense_communication(agwd, gnn, case, adj_m, num_dialogue_turns)
